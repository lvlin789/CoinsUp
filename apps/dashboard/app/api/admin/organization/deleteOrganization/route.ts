...
export async function DELETE(params: NextRequest) {
    ...
    try {
        ...
        return Response.json({ message: "Organization deleted successfully" }, { status: 200 });
    }
    catch (error: any) {
        console.error("Error deleting organization:", error);
        return Response.json({ error: error?.message || "Error deleting organization" }, { status: 500 });
    }
